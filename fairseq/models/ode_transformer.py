...
@register_model('ode_transformer')
class ODETransformerModel(FairseqModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--use-word-dropout', action='store_true', 
                            help='whether to use word dropout to prevent the overfitting problem')
        # fmt: on
    ...
    def build_model(cls, args, task):
        ...
        encoder = ODETransformerEncoder(args, src_dict, encoder_embed_tokens)
        decoder = ODETransformerDecoder(args, tgt_dict, decoder_embed_tokens)
        return ODETransformerModel(encoder, decoder)


@register_model('ode_transformer_lm')
class ODETransformerLanguageModel(FairseqLanguageModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--max-positions', type=int, metavar='N',
                            help='the max positions of target sentence')
        parser.add_argument('--max-relative-length', type=int, default=-1,
                            help='the max relative length')
        # fmt: on
    ...
    def build_model(cls, args, task):
        ...
        decoder = ODETransformerDecoder(
            args, task.output_dictionary, embed_tokens, no_encoder_attn=True, final_norm=False,
        )
        return ODETransformerLanguageModel(decoder)


class ODETransformerEncoder(FairseqEncoder):
    """
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        # word dropout
        if self.training and self.use_word_dropout:
            x = word_drop(x, self.word_dropout)
...
class ODETransformerDecoder(FairseqIncrementalDecoder):
    """
...
def word_drop(tensor, prob):
    bsz, sent_len, embed_dim = tensor.size()
    pos_sampling = torch.rand(bsz, sent_len, 1) > prob
    pos_sampling = pos_sampling.cuda()
    tensor = tensor.masked_fill(pos_sampling, value=0)
    return tensor


@register_model_architecture('ode_transformer_lm', 'ode_transformer_lm')
...
@register_model_architecture('ode_transformer_lm', 'ode_transformer_lm_big')
def ode_transformer_lm_big(args):
...
@register_model_architecture('ode_transformer_lm', 'ode_transformer_lm_wiki103')
def ode_transformer_lm_wiki103(args):
    ...
    args.dec_calculate_num = 2
    args.max_relative_length = 8
    ode_transformer_lm_big(args)


@register_model_architecture('ode_transformer_lm', 'ode_transformer_lm_gbw')
def ode_transformer_lm_gbw(args):
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
...
@register_model_architecture('ode_transformer_lm', 'ode_transformer_lm_single')
def ode_transformer_lm_single(args):
    #args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 512)
    args.dropout = getattr(args, 'dropout', 0.1)
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
    args.relu_dropout = getattr(args, 'relu_dropout', 0.1)
...
@register_model_architecture('ode_transformer', 'ode_transformer')
...
@register_model_architecture('ode_transformer', 'ode_transformer_t2t_wmt_en_de')
def ode_transformer_t2t_wmt_en_de(args):
...
def ode_relative_transformer_t2t_wmt_en_de(args):
    args.max_relative_length = 8
    ode_transformer_t2t_wmt_en_de(args)

@register_model_architecture('ode_transformer', 'ode_relative_transformer_t2t_wmt_en_de_big')
def ode_relative_transformer_t2t_wmt_en_de_big(args):
    ...
    args.dropout = getattr(args, 'dropout', 0.3)
    args.max_relative_length = 8
    ode_relative_transformer_t2t_wmt_en_de(args)