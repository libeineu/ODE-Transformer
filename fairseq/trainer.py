...
class Trainer(object):
    ...
    def init_meters(self, args):
        ...
        for i in range(args.encoder_layers):
            self.meters['enc{}_attn_input_gnorm'.format(i)] = AverageMeter()  # gradient norm
            self.meters['enc{}_ffn_fc1_gnorm'.format(i)] = AverageMeter()
    ...
    def train_step(self, samples, dummy_batch=False):
        ...
        try:
            ...
            # add_norm
            enc_attn_input_gnorm = []
            enc_ffn_fc1_gnorm = []
            for i in range(self.encoder_layers):
                enc_attn_input_weight = [v for k, v in self.model.named_parameters()
                                          if k == 'module.encoder.layers.{}.self_attn.in_proj_weight'.format(i)]
                enc_attn_input_gnorm.append(torch.nn.utils.clip_grad_norm_(enc_attn_input_weight, self.args.clip_norm))
                enc_ffn_fc1_weight = [v for k, v in self.model.named_parameters()
                                        if k == 'module.encoder.layers.{}.fc1.weight'.format(i)]
                enc_ffn_fc1_gnorm.append(torch.nn.utils.clip_grad_norm_(enc_ffn_fc1_weight, self.args.clip_norm))
            ...
            # add_norm
            for i in range(self.encoder_layers):
                self.meters['enc{}_attn_input_gnorm'.format(i)].update(enc_attn_input_gnorm[i])
                self.meters['enc{}_ffn_fc1_gnorm'.format(i)].update(enc_ffn_fc1_gnorm[i])
