...
class RegularizationCrossEntropyCriterion(label_smoothed_cross_entropy.LabelSmoothedCrossEntropyCriterion):
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        reg_loss = 0
        if not net_output[1]['enc_reg'] == 0:
            reg_loss = net_output[1]['enc_reg']
        if not net_output[1]['dec_reg'] == 0:
            reg_loss = reg_loss + net_output[1]['dec_reg']
        if reduce:
        ...
        loss = loss + reg_loss.type_as(loss)
